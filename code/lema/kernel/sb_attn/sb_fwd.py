...
def _forward(
    ...
    stride_wm,
    stride_wn,
    # lema patch: logit_scale is a RUNTIME scalar, not a tl.constexpr. Upstream made it a
    # constexpr, which is fine when alpha is folded into q (inv_temp is then always 1.0),
    # but we now pass the schedule's alpha and c directly -- and c ramps every step, so a
    # constexpr would re-JIT the kernel thousands of times per run.
    logit_scale,
    logit_bias,
    ...
    # lema patch: logit_bias is a PER-HEAD fp32 tensor (num_heads,) so each
    # head can carry its own -alpha_h * c (learnable-alpha support).
    qk_bias = inv_log2 * tl.load(logit_bias + head_pid)
...
def _fwd(q, k, v, logit_scale, logit_bias=0.0,
