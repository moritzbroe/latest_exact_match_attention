...
def _backward(
    ...
    DB_ptr,          # lema patch: per-(batch,head,prog) partial d_bias sums
    stride_dbb,
    stride_dbh,
    stride_dbm,
    logit_scale,
    logit_bias,   # lema patch: PER-HEAD (num_heads,) fp32 additive logit bias
    batch_size,
    token_size,
    ...
    qk_bias = inv_log2 * tl.load(logit_bias + head_pid)   # per-head, log2 units
    ...
    DB_prog_ptr = (DB_ptr + stride_dbb * batch_id + stride_dbh * head_id
                   + stride_dbm * seq_prog_id)
    _backward_one_row(
        seq_prog_id,
        seq_length,
        qk_scale,
...
def _bwd(do, dr, q, k, v, neg_log_acc, logit_scale, logit_bias=0.0,
         attend_current=False, BLOCK_M=64, BLOCK_N=32):
    batch_size, num_heads, token_size, dim_size = q.size()
    if not torch.is_tensor(logit_bias):
        logit_bias = torch.full((num_heads,), float(logit_bias),
                                dtype=torch.float32, device=q.device)
    else:
        logit_bias = logit_bias.to(device=q.device, dtype=torch.float32).contiguous()
    ...
    # lema patch: fp32 dk/dv — the lock-free atomic_add accumulation (see sb_varlen_bwd
    # locked_add patch) needs fp32 atomics; cast back to bf16 on return (same interface).
    ...
    db_part = torch.zeros((batch_size, num_heads, M_count),
                          dtype=torch.float32, device=q.device)
    ...
    return dq, dk.to(torch.bfloat16), dv.to(torch.bfloat16), db_part.sum(dim=(0, 2))
