...
def compute_block(
    ...
    # lema patch: default to the fp32 cumsum path. The tl.dot path casts log_om_beta to
    # bf16, whose 0.25-0.5 absolute rounding at log-magnitudes ~40-90 (large-alpha
    # hardening regime) turns into 20-40% attention-weight errors.
    use_cumsum: tl.constexpr = True,
    is_compiling: tl.constexpr = False,
    # lema patch: additive logit bias, in the same log2 units as qk_scale. Lets the LEMA
    # gate sigmoid(alpha*(<q,k> - c)) be expressed directly as scale*dot + bias, instead of
    # folding alpha and c into an extra q/k dimension. RUNTIME scalar, never a constexpr:
    # c ramps every step, and a constexpr would re-JIT the kernel on each new value.
    qk_bias=0.0,
):
    qk = tl.dot(q, tl.trans(k), allow_tf32=ALLOW_TF32) * qk_scale + qk_bias
    ...
    if on_band:  # diagonal
        ...
        if use_cumsum:
            log_p += tl.cumsum(log_om_beta, axis=1, reverse=True)  # lema patch: fp32, not q.dtype
        else:
...
def _forward_one_row(
    ...
    is_compiling: tl.constexpr = False,
    use_cumsum: tl.constexpr = True,  # lema patch: fp32 log-space accumulation (see compute_block)
    attend_current: tl.constexpr = False,
    qk_bias=0.0,  # lema patch: runtime additive logit bias (see compute_block)
):
    ...
    # BLOCK_M must be a multiple of BLOCK_N
    ...
    if NO_D_MASK:
        ...
        q = tl.load(
            Q_blk_ptrs, mask=M_mask[:, None] & D_mask[None, :], other=0.0)
...
def varlen_fwd(
    ...
    if return_attention:
        W = torch.full((num_heads, token_size, token_size), 0.0,
